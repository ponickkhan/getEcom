...
class StarTech extends Site
{
    public $baseUrl = 'https://www.startech.com.bd/';
    public $site = 'startech';
        ...
        foreach ($categories as $category) {
            ...
            while ($url) {
                ...
                if ($this->parser = $this->loadUrl($url)) {
                    ...
                    foreach ($this->parser->find('div.row div.layout_list_item div.wrap a') as $productLink) {
    ...
    public function fetchProductData($output = null)
    {
        ...
        foreach ($this->repository->getAllProducts($this->site, false, false, 'id, url') as $product) {
            ...
            if (($find = $this->parser->find('div#item_show_carousel a')) && isset($find[0])) {
                $images = [];

                foreach ($find as $img) {
                    if (substr($img->getAttribute('href'), -4) == '.jpg') {
                        $images[] = $img->getAttribute('href');
                    }
