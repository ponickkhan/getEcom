...
class Karintal extends Site
{
    public $baseUrl = 'https://www.karintal.com/shop';
    public $site = 'karintal';
    ...
    function fetchCategories()
    {
        ...
        $categories = [];
        foreach ($this->parser->find('div.header-main ul li a') as $link) {
            if ($link->href == '#') continue;

            $href = str_replace($this->baseUrl, '', $link->href);
            if(strpos($href, '/product-category') === 0) {
                $parsed = parse_url($href);
                $categories[$parsed['path']] = $link->plaintext;
            }
        }
