...
class Petcall extends Site
{
    public $baseUrl = 'http://www.petcall.co.il';
    public $site = 'petcall';
    ...
    {
        $this->parser = $this->loadUrlC($this->baseUrl . '/');

        $categories = [];
        foreach ($this->parser->find('div.megamenu a') as $link) {
            if (mb_strpos($link->href, '/category/', 0, 'UTF-8') !== false) {
    ...
    public function fetchProducts($output = null)
    {
        ...
        foreach ($categories as $category) {
            ...
            while (true) {
                ...
                $this->parser = $this->loadUrlC($url);

                try {
                    foreach ($this->parser->find('div.products_itemsgrid div.boxItem-wrap a.ee_product_click') as $productLink) {
    ...
    public function fetchProductData($output = false)
    {
        ...
        foreach ($this->repository->getAllProducts($this->site, false, false, 'id, url') as $product) {
            ...
            $productData = [];
            if (($find = $this->parser->find('div.item-name h1')) && isset($find[0])) {
                $productData['title'] = $find[0]->plaintext;
            }

           

            if (($find = $this->parser->find('div#product-thumbnails ul li a')) && isset($find[0])) {
                $images = [];
                $videos = [];
                foreach ($find as $img) {
                   if (substr($img->attr['href'], -4) == '.jpg') {
                    $images[] = $this->baseUrl .str_replace("_large", "", $img->attr['href']);
                   }else{

                    $videos[] = $img->attr['href'];
                       } 
                }
            }elseif (($find = $this->parser->find('div#imgBigDIV img')) && isset($find[0])) {
                 $images = [];
                 $videos = [];
                
                foreach ($find as $img) {
                   if (substr($img->attr['src'], -4) == '.jpg') {
                    $images[] = $this->baseUrl .str_replace("_large", "", $img->attr['src']);
                   }
                }
            }else{
                 $images = [];
            ...
            if (!empty($videos)) {
                $productData['video'] = implode(',', $videos);
            }
	    ...
	    } elseif (($find = $this->parser->find('div.product_description div.description')) && isset($find[0])) {
                $productData['description'] = $this->purifyContent($find[0]->innertext);
