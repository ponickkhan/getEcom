...
class Animalshop extends Site
{
    public $baseUrl = 'https://www.animalshop.co.il';
    public $site = 'animalshop';
    ...
    function fetchCategories()
    {
        ...
        $categories = [];
        $count = 0;
        echo(count($this->parser->find('div.megamenu ul.ul_menu-horizontal_image_top > li.switch_image')));
        foreach ($this->parser->find('div.megamenu ul.ul_menu-horizontal_image_top > li.switch_image') as $primeCategory) {
            if ($count == 6) {
                ...
                foreach ($primeCategory->find('li.second_layer a') as $atag) {
                  $url = str_replace($this->baseUrl, '', $atag->href);
                  $categories[$url] = $atag->text();
                }
            } else {
                foreach ($primeCategory->find('li.third_layer a') as $atag) {
                  $url = str_replace($this->baseUrl, '', $atag->href);
                  $categories[$url] = $atag->text();
                }
            }
    ...
    public function fetchProductData($output = null)
    {
        ...
        foreach ($this->repository->getAllProducts($this->site, false, false, 'id, url') as $product) {
            ...
            if (($find = $this->parser->find('ul#imageGallery li img')) && isset($find[0])) {
                $images = [];
                foreach ($find as $img) {
                    $images [] = $img->attr['src'];
                }
            ...
            if (($find = $this->parser->find('ul#imageGallery li[data-type="video"]')) && isset($find[0])) {
                $images = [];
                foreach ($find as $img) {
                    $productData['video'] = $img->attr['data-src'];
                }
            }

            if (($find = $this->parser->find('div#product-prices-div div.prices-box strike.reg-price span[itemprop="price"]')) && isset($find[0])) {
                $productData['price'] = floatval(str_replace([',','₪','NIS'], '',trim($find[0]->attr['content'])));
            }

            if (($find = $this->parser->find('div#product-prices-div div.prices-box p.sale-price span[itemprop="price"]')) && isset($find[0])) {
                $productData['sale_price'] = floatval(str_replace(',', '',trim($find[0]->attr['content'])));
            }


            if (($find = $this->parser->find('div.product-short-desc')) && isset($find[0])) {
            ...
            if (($find = $this->parser->find('ul.product-details-ul li[itemprop="brand"] div.label_wrap span.value')) && isset($find[0])) {
